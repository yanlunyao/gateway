...
/*
 * Function: mAliasPustToCb
 * return: success>>0,error>>error flag
 * description: none
*/
int mAliasPushToCb(int status)
{
	...
	status = status*(-1);
	// Push to CB_Daemon
	// generate push string
	if( (send_cb_len = cJsonAlias_callback(send_cb_string, status)) <0) {
		return clientAdminPushToCBDaemonErr;
	}
	//if push failed, return push error
	if ( (push_to_CBDaemon(send_cb_string, send_cb_len)) < 0 ) {
		return clientAdminPushToCBDaemonErr;
	}
	return 0;
}
/*
 * Function: mPswPushToCb
 * return: success>>0,error>>error flag
 * description: none
*/
int mPswPushToCb(int status)
{
	...
	// Push to CB_Daemon
	// generate push string
	if( (send_cb_len = cJsonPsw_callback(send_cb_string, status)) <0) {
		return clientAdminPushToCBDaemonErr;
	}
	//if push failed, return push error
	if ( (push_to_CBDaemon(send_cb_string, send_cb_len)) < 0 ) {
