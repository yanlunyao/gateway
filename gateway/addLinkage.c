...
//	wrtie database
	...
	res = add_linkage_db(&id_value, &lnk_st);
	if(res<0){
		goto all_over;
	}
	lnk_st.lnk_base.lid =  id_value;

all_over:
	db_close();
    api_response(res, lnk_st.lnk_base.lid, &lnk_st.lnk_base);
    //push to cb_daemon
	if((send_cb_len = cJsonLinkage_callback(send_cb_string, SUBID_ADD_LINK, res, lnk_st.lnk_base.lid, &lnk_st.lnk_base)) >=0) {
